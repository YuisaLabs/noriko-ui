...
// src/commands/add.ts
function addComponent(name, targetDir) {
  const componentMap = {
    button: import_path.default.resolve(__dirname, "../../../ui/src/components/button.tsx")
  };
  const src = componentMap[name];
  if (!src) {
    console.error(source_default.red(`Component ${name} not found.`));
    process.exit(1);
  }
  const destinationDir = import_path.default.resolve(process.cwd(), targetDir, `${name}.tsx`);
  if (import_fs_extra.default.existsSync(destinationDir)) {
    console.warn(source_default.yellow(`Component ${name} already exists in ${targetDir}.`));
    return;
  }
  import_fs_extra.default.ensureDirSync(import_path.default.dirname(destinationDir));
  import_fs_extra.default.copyFileSync(src, destinationDir);
  console.log(source_default.green(`Component ${name} added to ${targetDir}.`));
}

// src/index.ts
var program2 = new Command();
program2.name("noriko").description("Noriko UI CLI - Add UI Component to your project").version(version);
program2.command("add").argument("<component>", "Component name to add (e.g. button)").option("-d, --dir <directory>", "Target directory", "components").description("Add a component to your project").action((component, options) => {
